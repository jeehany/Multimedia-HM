...
while($row = mysqli_fetch_assoc($rs)){
  ...
  echo '<tr>';
  echo '<td>'.$no++.'</td>';
  echo '<td><strong>'.htmlspecialchars($row['nama_alat']).'</strong></td>';
  echo '<td>'.htmlspecialchars($row['jenis']).'</td>';
  echo '<td><span class="badge '.$kondisi_badge.'">'.ucfirst(htmlspecialchars($row['kondisi'])).'</span></td>';
  echo '<td>'.htmlspecialchars($row['lokasi']).'</td>';
  echo '<td>'.htmlspecialchars($row['penanggung_jawab']).'</td>';
  echo '<td>'.date('d M Y', strtotime($row['tanggal_pembelian'])).'</td>';
  if(can_edit()) {
    echo '<td class="text-center">
      <a class="btn btn-sm btn-primary me-1" href="alat_edit.php?id='.$row['id_alat'].'" title="Edit"><i class="fas fa-edit"></i></a>
      <a class="btn btn-sm btn-danger" href="?delete='.$row['id_alat'].'" onclick="return confirm(\'Hapus data ini?\')" title="Hapus"><i class="fas fa-trash"></i></a>
    </td>';
  }
  echo '</tr>';
}
if(mysqli_num_rows($rs) == 0){
  echo '<tr><td colspan="'.(can_edit()?8:7).'" class="text-center py-4 text-muted"><i class="fas fa-inbox me-2"></i>Tidak ada data ditemukan</td></tr>';
}
?>
        </tbody>
      </table>
