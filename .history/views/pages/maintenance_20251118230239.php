...
<div class="card mb-3">
  <div class="card-body">
    <form method="get" class="row g-2 auto-filter">
      <div class="col-md-3"><input name="q" value="<?=htmlspecialchars($_GET['q'] ?? '')?>" class="form-control" placeholder="Cari alat atau teknisi..."></div>
      <div class="col-md-2"><select name="status" class="form-select"><option value="">--Status--</option><option value="belum" <?=(!empty($_GET['status']) && $_GET['status']=='belum')?'selected':''?>>belum</option><option value="selesai" <?=(!empty($_GET['status']) && $_GET['status']=='selesai')?'selected':''?>>selesai</option></select></div>
      <div class="col-md-2"><select name="id_alat" class="form-select"><option value="">--Alat--</option><?php $ars=mysqli_query($conn,"SELECT id_alat,nama_alat FROM tabel_alat"); while($ar=mysqli_fetch_assoc($ars)){ echo '<option value="'.$ar['id_alat'].'" '.((!empty($_GET['id_alat']) && $_GET['id_alat']==$ar['id_alat'])?'selected':'').'>'.htmlspecialchars($ar['nama_alat']).'</option>'; } ?></select></div>
      <div class="col-md-2 align-self-end"><?php if(can_edit()): ?><a class="btn btn-success" href="maintenance_add.php"><i class="fa fa-plus"></i> Tambah</a><?php endif; ?></div>
    </form>
  </div>
</div>
