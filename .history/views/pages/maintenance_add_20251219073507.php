...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $status = $_POST['status'];
  $performed_at = ($status === 'selesai') ? mysqli_real_escape_string($conn,$tanggal) : NULL;
  mysqli_query($conn, "INSERT INTO tool_maintenances (tool_id,maintenance_type_id,performed_by,performed_at,notes,cost,created_at) VALUES ($id_alat, NULL, '".mysqli_real_escape_string($conn,$teknisi)."', ".($performed_at? "'".$performed_at."'" : "NULL").", '".mysqli_real_escape_string($conn,$jenis)."', $biaya, '".mysqli_real_escape_string($conn,$tanggal)."')");
  ...
  <div class="card mx-auto" style="max-width:800px;margin-top:30px">
    <div class="card-body">
      ...
      <form method="post" class="row g-3">
        <div class="col-md-6"><label class="form-label">Alat</label><select name="id_alat" class="form-select"><?php $rs=mysqli_query($conn,"SELECT id,tool_name FROM tools"); while($r=mysqli_fetch_assoc($rs)){ echo '<option value="'.$r['id'].'">'.htmlspecialchars($r['tool_name']).'</option>'; } ?></select></div>
