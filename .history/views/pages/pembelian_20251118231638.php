...
<div class="card mb-3"><div class="card-body">
  <form method="get" class="row g-2 auto-filter">
    ...
    <div class="col-md-1 align-self-end"><a class="btn btn-success w-100" href="pembelian_add.php"><i class="fa fa-plus"></i> Tambah</a></div>
  </form>
</div></div>
<?php elseif(can_edit()): ?>
...
<div class="card mb-3"><div class="card-body">
  <form method="get" class="row g-2 auto-filter">
    ...
    <div class="col-md-2"><input type="date" name="tgl_akhir" class="form-control" value="<?=htmlspecialchars($_GET['tgl_akhir'] ?? '')?>" title="Tanggal Akhir"></div>
    <div class="col-md-2"><select name="status" class="form-select"><option value="">--Status--</option><option value="menunggu" <?=(!empty($_GET['status']) && $_GET['status']=='menunggu')?'selected':''?>>menunggu</option><option value="disetujui" <?=(!empty($_GET['status']) && $_GET['status']=='disetujui')?'selected':''?>>disetujui</option><option value="ditolak" <?=(!empty($_GET['status']) && $_GET['status']=='ditolak')?'selected':''?>>ditolak</option></select></div>
    <div class="col-md-1 align-self-end"><a class="btn btn-success" href="pembelian_add.php"><i class="fa fa-plus"></i> Tambah</a></div>
  </form>
</div></div>
<?php else: ?>
