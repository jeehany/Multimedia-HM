...
<form method="get" class="row g-2 auto-filter">
  <div class="col-md-3"><input name="q" value="<?=htmlspecialchars($_GET['q'] ?? '')?>" class="form-control" placeholder="Cari nama atau keterangan..."></div>
  <div class="col-md-2"><input type="month" name="bulan" class="form-control" value="<?=htmlspecialchars($_GET['bulan'] ?? '')?>"></div>
  <div class="col-md-2"><select name="jenis" class="form-select"><option value="">--Jenis--</option><option value="pembelian" <?=(!empty($_GET['jenis']) && $_GET['jenis']=='pembelian')?'selected':''?>>pembelian</option><option value="maintenance" <?=(!empty($_GET['jenis']) && $_GET['jenis']=='maintenance')?'selected':''?>>maintenance</option></select></div>
  <div class="col-md-2 align-self-end"><?php if(can_edit()): ?><a class="btn btn-success" href="pengeluaran_add.php"><i class="fa fa-plus"></i> Tambah</a><?php endif; ?></div>
</form>
</div></div>
