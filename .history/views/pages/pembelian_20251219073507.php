...
if(can_approve() && isset($_GET['approve']) && isset($_GET['status_baru'])){
    ...
    mysqli_query($conn, "UPDATE purchases SET status='$status_baru' WHERE id=$id");
    header('Location: pembelian.php');
    exit;
}
...
$sql = "SELECT * FROM purchases" . (count($where)? ' WHERE '.implode(' AND ',$where): '') . " ORDER BY id DESC";
...
while($r=mysqli_fetch_assoc($q)){
  ...
  echo '<tr>';
  echo '<td>'.$no++.'</td>';
  echo '<td><strong>'.htmlspecialchars($r['purchase_code'] ?? '-') .'</strong></td>';
  echo '<td>Rp '.number_format($r['total_amount'] ?? 0,0,',','.').'</td>';
  echo '<td>'.date('d M Y', strtotime($r['purchase_date'] ?? $r['created_at'] ?? 'now')).'</td>';
  echo '<td>'.$status_badge.'</td>';
  
  if(can_approve()) {
      echo '<td class="text-center">';
        if($r['status'] === 'menunggu') {
          echo '<a class="btn btn-sm btn-success me-1" href="?approve='.$r['id'].'&status_baru=disetujui" onclick="return confirm(\'Setujui permohonan ini?\')" title="Setujui"><i class="fas fa-check"></i></a>';
          echo '<a class="btn btn-sm btn-danger" href="?approve='.$r['id'].'&status_baru=ditolak" onclick="return confirm(\'Tolak permohonan ini?\')" title="Tolak"><i class="fas fa-times"></i></a>';
      } else {
      ...
      echo '<td class="text-center">';
        echo '<a class="btn btn-sm btn-primary me-1" href="pembelian_edit.php?id='.$r['id'].'" title="Edit"><i class="fas fa-edit"></i></a>';
        echo '<a class="btn btn-sm btn-danger" href="?delete='.$r['id'].'" onclick="return confirm(\'Hapus data ini?\')" title="Hapus"><i class="fas fa-trash"></i></a>';
      echo '</td>';
  }
  echo '</tr>';
}
